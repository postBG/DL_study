...
class Node(object):
    ...
    def __init__(self, inbound_nodes=None):
        ...
        # Note: dictionary of inbound node's partial derivative to cost
        self.gradients = {}
...
class Add(Node):
    ...
    def forward(self):
        self.value = np.sum([node.value for node in self.inbound_nodes])
    ...
    def backward(self):
        ...
        for n in self.outbound_nodes:
            ...
            for node in self.gradients.keys():
                self.gradients[node] += grad_cost
